...
これはWindowsのSecure Desktop(UAC昇格ダイアログ・ログイン画面、Part 7既述)と同種の、OSが意図的に設けているセキュリティ境界であり、ロック画面より上に通常権限プロセスのウィンドウを合成させない仕組みが働いていると考えられる。3M-2(無人アクセス)がスコープ外である今回の対象(3M-1、在席)では、「画面がロックされていない」ことが前提となる。

**画面ロック解除後(同日、ユーザーがロック解除)に再検証し、この仮説どおりであることを確認した**: `CGSessionCopyCurrentDictionary`から`CGSSessionScreenIsLocked`キー自体が消えたこと(ロック時のみ存在するキー)でロック解除を確認した上で同じ手順(署名済み`SardpServer.app`+`SardpClient.app`、`--display window`)を再実行し、クライアント側の`--dump-frames`でサーバーの実キャプチャを取り出して`ffmpeg`でデコードしたところ、実デスクトップ(メニューバー・Dock・複数のウィンドウ)の中に、タイトルバーに`SARDP 192.168.1.6:4433 (3420x2224)`と表示された`SardpClient`のウィンドウ自体が実際に合成されている画像が得られた(ウィンドウの中身はデスクトップの実況ミラーのため、ウィンドウ自身が入れ子に映り込む再帰的な見た目になる)。**3M-1-eの受け入れ基準(スクリーンショットでの目視確認)はこれで満たされた。**

**副次的に判明したこと**: 最初にこの確認に使っていた自作の`Shooter`ツール(署名なし・`open`を経由せず直接実行、`SCScreenshotManager.captureImage(in:)`を使用)は、ロック中・ロック解除後の両方で**バイト単位で同一の画像**(暗い単色、144849バイト)を返し続けており、実際の画面内容を反映していなかった。ロック解除後も同じ結果が出たことで、これは「ロック画面を捉えていた」のではなく**このツール自体が信頼できない**(署名なし直接実行という経路の何らかの制約で、キャッシュされた/無効な画像を返し続けていたと見られる)ことが分かった。当初「ロック画面の壁紙を正しく捉えている」と解釈していたのは誤りで、実際にロック状態を確認できていたのは`CGSessionCopyCurrentDictionary`とサーバー側`--dump-frames`(署名済みバンドル経由、本物の許可を使う経路)の方だった。**目視確認が必要な場合は、必ず署名済み`.app`バンドル経由(`open`起動)の`--dump-frames`+`ffmpeg`デコードを使うこと** — 署名なし直接実行のスクリーンショットツールは今回の環境では信頼できない。
...
**未対応のまま残っている点**(Windows項目16と同じ形で記録): ウィンドウのリサイズ非対応(固定サイズ)、macOSのウィンドウは表示専用でキーボード・マウス入力を一切捕捉しない(3M-1-d以来の`--input-script`が引き続き唯一の入力経路、Windowsの`--input on`相当は未実装)、ストリーム解像度変更時のデコーダ再作成は`decoder_dims`比較のみでWindows同様未検証、Tier 4(ソフトウェア)フォールバックなし。
